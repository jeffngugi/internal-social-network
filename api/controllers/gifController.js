...
class gifController {
  static async postGif(req, res) {
    ...
    await cloudinary.uploader
      ...
      .then(image => {
        client
          .query(
            "INSERT INTO gifs(created_on, gif_url, author_id, public_id, title)VALUES($1,$2,$3,$4, $5)",
            [uniqueFilename, image.url, id, image.public_id, title]
          )
  ...
  static async getAll(req, res) {
    await client.query("SELECT * FROM gifs").then(gifs => {
      if (gifs.rows.length > 0)
        responder.responseSuccess(
          res,
          `${gifs.rows.length} gifs found`,
          gifs.rows
        );
  ...
  static async deleteGif(req, res) {
    const { id } = req.user.rows[0];
    try {
      const gif = await client.query("SELECT * FROM gifs WHERE gif_id=$1", [
      ...
      await cloudinary.uploader
        ...
        .then(result => {
          if (result.result === "not found")
            return res.json("Not found in database");
          else {
            client
              .query("DELETE FROM gifs WHERE gif_id=$1", [
                parseInt(req.params.id)
              ])
